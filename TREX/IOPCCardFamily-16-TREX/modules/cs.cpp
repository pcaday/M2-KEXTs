			  ...
			  ioaddr_t num, u_int lines, char *name)
{
	...
	_try = s->io[i].BasePort + s->io[i].NumPorts;
	if ((*base == 0) || (*base == _try))
	    if (find_io_region(&_try, num, 0, name) == 0) {
		*base = _try;
		s->io[i].NumPorts += num;
		s->io[i].InUse += num;
		break;
	...
	_try = s->io[i].BasePort - num;
	if ((*base == 0) || (*base == _try))
	    if (find_io_region(&_try, num, 0, name) == 0) {
		s->io[i].BasePort = *base = _try;
		s->io[i].NumPorts += num;
		s->io[i].InUse += num;
		break;
	    }
...
static int cs_request_irq(client_handle_t handle, irq_req_t *req)
{
	...
	ret = CS_IN_USE;
	if (req->IRQInfo1 & IRQ_INFO2_VALID) {
	    u_int _try, mask = req->IRQInfo2 & s->cap.irq_mask;
	    for (_try = 0; _try < 2; _try++) {
		for (irq = 0; irq < 32; irq++)
		    if ((mask >> irq) & 1) {
			ret = try_irq(req->Attributes, irq, _try);
			if (ret == 0) break;
		    }
		if (ret == 0) break;
	    }
