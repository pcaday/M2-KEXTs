...
// begin modifications for TREX: had to switch to C++ because of 10.0/10.2 header issues
//  and g++2 does not handle this labeled initializer
#if 0
...
#else
static struct timer_list poll_timer = {
    0, pcic_interrupt_wrapper, 0
};
#endif
// end modifications for TREX
...
static int pci_lookup(u_int _class, pci_id_t *id,
		      u_char *bus, u_char *devfn)
{
    pci_info info;
    while (pci->get_nth_pci_info((*id)++, &info) == 0) {
	if (((info.class_base<<8) + info.class_sub) == _class) {
	    *bus = info.bus;
...
static int __init pci_lookup(u_int _class, pci_id_t *id,
			     u_char *bus, u_char *devfn)
{
    if ((*id = pci_find_class(_class<<8, *id)) != NULL) {
	*bus = (*id)->bus->number;
	*devfn = (*id)->devfn;
	return 0;
    } else return -1;
...
#ifdef __LINUX__ //macosx
static void __init pci_probe(u_int _class)
{
    socket_info_t *s = &socket[sockets];
    u_short i, v, d;
    pci_id_t id;
    ...
    id = 0;
    while (pci_lookup(_class, &id, &s->bus, &s->devfn) == 0) {
	...
	if (((i < PCIC_COUNT) && (pcic[i].flags & IS_CARDBUS)) ||
	    (_class == PCI_CLASS_BRIDGE_CARDBUS))
	    add_cb_bridge(i, v, d);
	else
...
pci_probe(u_int _class)
{
    ...
    if (_class == ((base_class << 8) | sub_class)) {
	pci_readw(s, PCI_VENDOR_ID, &v);
	pci_readw(s, PCI_DEVICE_ID, &d);
	for (i = 0; i < PCIC_COUNT; i++)
	    if ((pcic[i].vendor == v) && (pcic[i].device == d)) break;
