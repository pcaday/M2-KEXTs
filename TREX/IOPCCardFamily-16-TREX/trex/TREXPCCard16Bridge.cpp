...
TREXPCCard16Bridge::addNubInterruptProperties(OSDictionary * propTable)
{
    ...
    if (!specifier) {
        VerboseIOLog("TREX: could not create property\n");
        return;
    }
...
trex_fixup(IOPCCard16Device *dev, unsigned int socket)
{
    ...
    if (trex_get_ranges(socket, range_list, &num_ranges))
        return false;
...
#else
    for (i = 0; i < memWinCnt; i++) {
        new_ranges[i].start = range_list[2*i];
        new_ranges[i].length = range_list[2*i + 1] - range_list[2*i] + 1;
    }
    
    range_ptr = &new_ranges[memWinCnt];
    for (i = 0; i < ioWinCnt; i++) {
        range_ptr[i].start = range_list[2 * (i + 4)];
        range_ptr[i].length = range_list[2 * (i + 4) + 1] - range_list[2 * (i + 4)] + 1;
    }
#endif
