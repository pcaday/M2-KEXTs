...
#define DEBUG_ACTION(n, e) if (pc_debug>(n)) e
#define VerboseIOLog(x...) if (pc_debug>1) IOLog(x)
#else
#define DEBUG(n, args...) do { } while (0)
#define DEBUG_ACTION(n, e) do { } while (0)
#define VerboseIOLog(x...) do { } while (0)
#endif
...
#define trex_cycle_time 40	// in ns -- valid for PowerBook 1400
                                //  Seems to be 60ns in some PowerBooks...
                                //  I don't know which
...
// With the new interrupt scheme, these should actually never be called
static u_int
...
static int trex_service(u_int sock, u_int cmd, void *arg)
{
    ...
    DEBUG(3, "trex socket services: service(%d, %d, 0x%p)\n", sock, cmd, arg);
...
int trex_get_ranges(unsigned int socket_number, unsigned long *range_list, unsigned int *num_ranges, unsigned int *num_mranges)
{
    unsigned int map, memMaps = 0, nmaps = 0;
    TREXSocket *s;
