...
// begin TREX modification

//   TREX does attribute memory differently than most controllers
static void cis_read_write_done(socket_info_t *s)
{
    static const pccard_mem_map doneSig = {-1, 0, 0, 0, 0};
    
    s->ss_entry(s->sock, SS_SetMemMap, (pccard_mem_map *) &doneSig);
}
// end TREX modification
		  ...
		  u_int len, void *ptr)
{
    ...
    // begin TREX modification
    cis_read_write_done(s);

#if 0    
    IOLog("cs: read_cis_mem(%d, %#x, %u) ->  %#2.2x %#2.2x %#2.2x %#2.2x ...\n", attr, addr, len,
