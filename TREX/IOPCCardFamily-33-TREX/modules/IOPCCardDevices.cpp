...
// Need to call trex_fixup to get the right IODeviceMemory data
extern bool trex_fixup(IOPCCard16Device *dev, unsigned int socket);
...
IOPCCard16Device::configure(UInt32 index = 0)
{
    ...
    if (!enabler->configure(index))
        return false;
    
    return trex_fixup(this, socket);
}
