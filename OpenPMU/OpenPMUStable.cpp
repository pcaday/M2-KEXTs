...
//    theHWInterface = new OpenViaInterface;		// polled interface
    ...
    pmuInterrupt = IOInterruptEventSource::interruptEventSource(this, pmuInterruptCaller, intsNub, isM2 ? pmu_int_index : VIA_DEV_VIA2);
...
//	Verbose_IOLog("OpenPMUInterface: got PMUFirmware = %x\n", iBuffer);
    } else {
...
OpenPMUInterface::wakeUp()
{
    ...
    // Powers on the LCD
    // We need this only for PwerbookG31999 (and M2) that in this moment does not have
    // a brightness driver:
	platProv = getPlatform()->getProvider();
    if (IODTMatchNubWithKeys(platProv, "'PowerBook1,1'")
         /* || IODTMatchNubWithKeys(platProv, "'AAPL,1400'") */)
...
OpenPMUInterface::hostIsMobile()
{
    ...
    if (topProvider != NULL) {
		...
		if (strncmp (topProviderName, "AAPL,1400", 7) == 0) return true;
	}
