...
OpenPMUInterface::start(IOService *nub)
{
    ...
    while (provider != NULL) {
        ...
        if (s != NULL) {
            // Check for an M2-compatible PMU
//            if (s->isEqualTo("via-pmu", 7)) {
                if (compat && compat->isEqualTo("pmu-m2", 6)) {
    #if VERBOSE
                    IOLog("OpenPMUInterface::start found M2 hardware\n");
    #endif
                    isOldHardware = isM2 = true;
                    break;
                }
//            }
            
            if (s->isEqualTo("mac-io", 6)) {
                ...
                isOldHardware = ((compat == NULL) || compat->isEqualTo("heathrow", 7) || compat->isEqualTo("paddington", 10));
    #if VERBOSE
...
// -- new VIA/PMU structure
    // For M2, calculate the bus and cpu speeds (taken from AppleVIA).
    if (isM2) {
        super::callPlatformFunction("GetDefaultBusSpeeds", false, &numSpeeds, &speedList, 0, 0);
        PE_Determine_Clock_Speeds(viaMap->getVirtualAddress(), numSpeeds, speedList);
    }
...
//    theHWInterface = new OpenViaInterface;		// polled interface
...
// -- new VIA/PMU structure
#if 0
    if (isM2) {
        intsNub = intsNub->getProvider();
        if (!intsNub) {
            IOLog("OpenPMUInterface::start() could not get intsNub (2)\n");
            return false;
        }
        intsNub = intsNub->getProvider();
        if (!intsNub) {
            IOLog("OpenPMUInterface::start() could not get intsNub\n");
            return false;
        }
    }
#endif

    pmuInterrupt = IOInterruptEventSource::interruptEventSource(this, pmuInterruptCaller, intsNub, isM2 ? pmu_int_index_m2 : VIA_DEV_VIA2);
   ...
   // and adds it to the workloop:
    ...
    if (sendMiscCommand (kPMUreadPmgrVers, (IOByteCount)0, NULL, &iLen, &iBuffer) == kIOReturnSuccess) {
        ...
        Verbose_IOLog("OpenPMUInterface: could not get PMUFirmware\n");
    }
...
{
#ifdef VERY_VERBOSE_LOGS_ON_PMU_INT
...
//    Verbose_IOLog("OpenPMUInterface::sendMiscCommand() command = %02x\n", command);
//    IOSleep(5000);
...
{
    // This will prevent use of the interrupt-based
    //  interface -- this is important since interrupts are
...
OpenPMUInterface::preSleepSequence()
{
    ...
    // a brightness driver:
    //  also for M2 on Jaguar
        
    platProv = getPlatform()->getProvider();
    if (IODTMatchNubWithKeys(platProv, "'PowerBook1,1'")
    #ifdef _JAGUAR
            || IODTMatchNubWithKeys(platProv, "'AAPL,1400'")
    #endif    
    )
        setLCDPower(false);
...
OpenPMUInterface::wakeUp()
{
    ...
    // Allow use of interrupt-based interface again
...
OpenPMUInterface::setInterrupts(bool on)
{
    ...
    if (on) {
        ...
        // disable the PMU interrupts from the HW interface
...
// ****** NOTE: 0x2 is SCC power, 0x1 is IWM (SWIM2, also?) power -- OF
...
//
// Purpose:
//          get the media bay ID (M2)
UInt8
OpenPMUInterface::readMBHWID()
{
    if (!isM2)
        return 0;
    ...
    if (sendMiscCommand(kPMUDoMediaBayDisp, (IOByteCount) sizeof(oBuffer), &oBuffer, &iLen, &iBuffer)
            != kIOReturnSuccess)
        return 0;
...
//
// Purpose:
//          read one or more bytes from XPRAM
IOReturn
OpenPMUInterface::readXPRAM(unsigned int offset, UInt8 *buffer, unsigned int len)
{
    UInt8 iBuffer[2];
    IOByteCount actualLen;
    IOReturn pmuResult;
    ...
    iBuffer[0] = offset;
    iBuffer[1] = len;
    actualLen = (IOByteCount) len;
    
    pmuResult = sendMiscCommand(kPMUxPramRead, (IOByteCount) sizeof(iBuffer), &iBuffer[0], &actualLen, buffer);
    
    if (pmuResult != kIOReturnSuccess)
        return pmuResult;
    if (actualLen != len)
...
//
// Purpose:
//          write one or more bytes to XPRAM
IOReturn
OpenPMUInterface::writeXPRAM(unsigned int offset, UInt8 *buffer, unsigned int len)
{
    UInt8 iBuffer[0xFF];
    
    if ((offset + len) > 0x100)
        return kIOReturnBadArgument;
    if (len > 0xFD)				// because the input buffer has maximum length 0xFF
        return kIOReturnBadArgument;
    if (!len)
        return kIOReturnSuccess;
    ...
    iBuffer[0] = offset;
    iBuffer[1] = len;
    bcopy(&iBuffer[2], buffer, len);
    
    return sendMiscCommand(kPMUxPramWrite, (IOByteCount) (len + 2), &iBuffer[0], NULL, NULL);
}
...
//              param->command, param->sLength,  param->sBuffer, param->rLength, param->rBuffer);
    ...
    else if (functionName->isEqualTo(kReadMBHWID)) {
        if (param1 == NULL)
            return kIOReturnBadArgument;
        *((UInt8 *) param1) = readMBHWID();
		return kIOReturnSuccess;
    }
...
OpenPMUInterface::hostIsMobile()
{
    ...
    if (topProvider != NULL) {
		...
		if (strncmp (topProviderName, "AAPL,1400", 9) == 0) return true;
	}
...
OpenPMU::start(IOService *nub)
{
    ...
    if (isM2) {
        Verbose_IOLog("OpenPMU::start() creating no-nvram property\n");
	nub->setProperty("no-nvram", true);		// No NVRAM on M2
    }
...
*/
    ...
    // Create the XPRAM controller (M2)
    if (isM2) {
        ourXPRAMinterface = new OpenPMUXPRAMController;
    
        if (ourXPRAMinterface != NULL) {
            if (ourXPRAMinterface->init(0,this) &&
                ourXPRAMinterface->attach(this) &&
                ourXPRAMinterface->start(this)) {
    #if VERBOSE
                IOLog("OpenPMU::allocateInterfaces ourXPRAMinterface is up and running\n");
    #endif
            }
            else {
    #if VERBOSE
                IOLog("OpenPMU::allocateInterfaces ourXPRAMinterface is not working\n");
    #endif
//                success = false;
            }
        }
...
*/
    ...
    if (ourXPRAMinterface != NULL) {
        ourXPRAMinterface->stop(this);
        ourXPRAMinterface->release();
        ourXPRAMinterface = NULL;
    }
    else
        success = false;
