...
OpenViaInterface::start(IOService *provider)
{
    ...
    if (success) {
		...
		// Get whether we are on M2
		isM2 = pmuParent->isM2;

                interruptSource = pmuParent->getProvider();

// -- new VIA/PMU structure
#if 0
                if (isM2) {
                    // Go all the way back to the via-pmu nub
                    if (!interruptSource) {
			IOLog("OpenViaInterface: could not get interruptSource (3)\n");
			return false;
                    }
                    interruptSource = interruptSource->getProvider();
                    if (!interruptSource) {
			IOLog("OpenViaInterface: could not get interruptSource (2)\n");
			return false;
                    }
                    interruptSource = interruptSource->getProvider();
                }
		...
		if (!interruptSource) {
			IOLog("OpenViaInterface: could not get interruptSource\n");
			return false;
		}

#ifdef VERBOSE_LOGS_ON_VIA                
                Verbose_IOLog("OpenViaInterface::start() interruptSource is %s\n", interruptSource->getName());
#endif
...
OpenViaInterface::hwInit(UInt8 *baseAddress)
{
	...
	
	{
		...
		if (isM2) {
			VIA2_dataB = baseAddress + 0x2000;   // misc data ack bits
			PMreq = (0x01 << M2Req);
			PMack = (0x01 << M2Ack);		
		} else {
			VIA2_dataB = baseAddress + 0x0000;
...
inline int OpenViaInterface::getSRInterruptNumber()
{
    int n = isM2 ? sr_int_index_m2 : VIA_DEV_VIA0;

    return n;
}

inline int OpenViaInterface::getPMUInterruptNumber()
{
    int n = isM2 ? pmu_int_index_m2 : VIA_DEV_VIA2;
...
OpenIntrrViaInterface::disableSRInterrupt ( void )
{
	...
	interruptSource->disableInterrupt(getSRInterruptNumber());
...
{
    if (interruptSource != NULL)
        interruptSource->enableInterrupt(getSRInterruptNumber());
...
OpenIntrrViaInterface::sendToPMU(PMUrequestPtr theRequest)
{
	...
	//  ******** NOTE: in nbpmac-linux, this is not done for M2 PMUs
    if (!waitForAck(true, 32)) {
#ifdef VERBOSE_LOGS_ON_VIA_INTR
...
OpenIntrrViaInterface::actUponState()
{
    ...
    if (transferState.currentInterruptState == kInterfaceIdle) {
        // BEGIN addition 12/17/08
        // read shift reg to clear SR int
        UInt8 lostRead = *VIA1_shift;
        lostRead++;			// Just in case the compiler starts getting ideas...
        eieio();
        // END addition
        return;
    }
        ...
        case kSwitchToRead:
            ...
            *VIA2_dataB |= PMreq;
            ...
            if ( rspLengthTable[transferState.currentTransfer->pmCommand] < 0 ) {
                ...
                // If the return length is fixed, subtract 1 to get the actual return length.
...
//    if (interruptSource != NULL) {
        IOReturn ret = interruptSource->registerInterrupt(getSRInterruptNumber(), this, (IOInterruptAction) shiftRegisterInt);
...
//            panic("interruptSource->registerInterrupt fails\n");
            Verbose_IOLog("OpenIntrrViaInterface::start() could not register interrupt, err = %s", stringFromReturn(ret));
            return false;
        }
//    }
//    else
...
{
    if (interruptSource != NULL)
        interruptSource->unregisterInterrupt(getSRInterruptNumber());
